...
sap.ui.controller("bdhreport.push_validation_report", {
/**
* Called when a controller is instantiated and its View controls (if available) are already created.
* Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
* @memberOf bdhreport.push_validation_report
*/
	...
	onShowSearchOptions: function(){
		...
		$.ajax({
		    ...
		    success : function(data, textStatus, jqXHR){
		    	...
		    	data.dataList.push({vora_version: "2.4.1"});
		    	
		    	data.dataList.push({vora_version: "2.6.1"});
		    	data.dataList.push({vora_version: "2.5.1"});
		    	...
		    	versionModel.setData(versionData);		    	
			   	...
			   	if (that.getView().byId('branchList').getFirstItem().getKey() == "master"){
			   		that.getView().byId('versionList').setSelectedKey(currentMasterVersion);
					that.getView().byId('versionList').setEnabled(false);
			   	}
		    },
		    error : function(jqXHR, textStatus, errorThrown) {		    	
			    sap.m.MessageToast.show("Error");
	...
	onBranchSelectChange: function(oEvent) {
		var that = this;
		var branch = oEvent.getParameter("selectedItem").getKey();
		if (branch == "master"){
			that.getView().byId('versionList').setSelectedKey(currentMasterVersion);
			that.getView().byId('versionList').setEnabled(false);
		} else{
			that.getView().byId('versionList').setEnabled(true);
		}
		
		var version = that.getView().byId("versionList").getSelectedKey();
		var branch = that.getView().byId("branchList").getSelectedKey();
		
        
        postData = {environment:{}};
        postData.environment.GERRIT_PROJECT = project;
        if(version!='' && version!="ALL"){postData.environment.VORA_VERSION = version;}
        if(branch!='' && branch!="ALL"){postData.environment.GERRIT_CHANGE_BRANCH = branch;}
        if (catagory == "BDH Nightly Validation") {
        	postData.environment.USE_FOR = "NIGHTLY_VALIDATION";
	...
	onUpdateFinished : function (oEvent) {
		...
		var oModel =  that.getView().byId('tableTitlePush').getModel();			    	
	...
	toCaseDetail:function(evt){
		var that = this;
		var reportTableController = that.getView().byId('reportTableForPushV');
		...
		if(evt.oSource.getBindingContext().getObject()){			
			...
			$.ajax({
				...
				dataType:"json",
	  			success : function(data, textStatus, jqXHR){
	  				var currentPageView = that.getSplitContObj().getPage("case_detail_push");
	  	        	var tableController = currentPageView.byId('caseListPush'); 
					...
					var oModel = new sap.ui.model.json.JSONModel();
				   	...
				   	tableController.setModel(oModel);
	  				that.getSplitContObj().toDetail("case_detail_push", data);	
