...
source ${BASE_FOLDER}/infrabox_stable_custom_commit_id.sh
...
if [ -d /tmp/hanalite-releasepack_continues_validation_stable ]; then
  rm -rf /tmp/hanalite-releasepack_continues_validation_stable/*
else
  mkdir -p /tmp/hanalite-releasepack_continues_validation_stable
  git clone ssh://InfraBox@git.wdf.sap.corp:29418/hanalite-releasepack
  pushd /tmp/hanalite-releasepack_continues_validation_stable/hanalite-releasepack
  git checkout -qf ${COMMIT_ID_STABLE}
  popd
f
fi

pushd /tmp/hanalite-releasepack_continues_validation_stable
cp ${BASE_FOLDER}/infrabox_stable.json infrabox.json
