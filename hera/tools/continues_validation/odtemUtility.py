...
class ODTEMApi:
    ...
    def checkCompName(self, comp_name):
        ...
        comp_output_json = requests.get(comp_url, verify=False)
        ...
        try:
            r = requests.post(insert_url, data=json.dumps(comp_info), headers=headers, verify=False)
        except:
    ...
    def getTestPlan(self, plan_name):
        ...
        testplan_info = requests.get(url, verify=False)
    ...
    def updateTestPlan(self, test_plan):
        ...
        try:
            r = requests.post(url, data=json.dumps(updata_data), headers=headers, verify=False)
        except :
            raise Exception("Fail to call odtem restApi.")
        if r.status_code != 200:
