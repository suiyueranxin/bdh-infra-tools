...
pushd /tmp/${3}
    ...
    if [ -d /project/dis_metadata/${3} ]; then
        rm  -rf /project/dis_metadata/${3}/*
    fi
    if [ -n "${4}" ];then
    ...
    if [ -f ${metadata_file_path} ]; then
        cp  ${metadata_file_path} /project/dis_metadata/${3}/
    fi
    if [ -d ${metadata_file_path} ]; then
        cp -r ${metadata_file_path}/* /project/dis_metadata/${3}/
    fi
    rm -rf /tmp/${3}/*
popd